...
keys (Node k Nothing l E E) = keys l
keys (Node k v l E E) = keys l ++ [[k]]

keys (Node k Nothing E m E) = (map (\x -> k : x) (keys m))
keys (Node k v E m E) = [[k]] ++ (map (\x -> k : x) (keys m))

keys (Node k Nothing E E r) = keys r
keys (Node k v E E r) = [[k]] ++ keys r

keys (Node k Nothing l m E) = keys l ++ (map (\x -> k : x) (keys m))
keys (Node k v l m E) = keys l ++ [[k]] ++ (map (\x -> k : x) (keys m))

keys (Node k Nothing E m r) = (map (\x -> k : x) (keys m)) ++ keys r
keys (Node k v E m r) = [[k]] ++ (map (\x -> k : x) (keys m)) ++ keys r
